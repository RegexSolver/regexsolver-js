...
describe("RegexSolverClient", () => {
  ...
  beforeEach(() => {
    ...
    // FIX: Mock the specific Axios instance created inside the client,
    // otherwise MockAdapter only intercepts the global `axios` object.
  ...
  test("getCardinality should work", async () => {
    ...
    expect(term._cardinality).toBe(cardinality);
  });
  ...
  test("getLength should work", async () => {
    ...
    expect(term._length).toBe(length);
  });
  ...
  test("error mapping should work for 400 Bad Request", async () => {
    ...
    // FIX: Updated to InvalidJsonError
    await expect(client.getCardinality(term)).rejects.toThrow(
  ...
  test("error mapping for 401 Unauthorized - Invalid Token", async () => {
    ...
    // FIX: Updated to InvalidTokenError
  ...
  test("error mapping for 403 Forbidden - Quota Exceeded", async () => {
    ...
    // FIX: Updated to QuotaExceededError
    await expect(client.getCardinality(Term.regex("abc"))).rejects.toThrow(
